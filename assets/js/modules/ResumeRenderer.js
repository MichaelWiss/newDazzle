...
export class ResumeRenderer {
  ...
  static renderHeader(parent, headerData) {
    ...
    const h1 = DOMUtils.createElement("h1", "resume-name", {
    ...
    if (headerData.links) {
      ...
      headerData.links.forEach((link) => {
        ...
        if (link.isButton) {
          const btn = DOMUtils.createElement("button", "download-btn", {
            text: link.text,
          });
          btn.onclick = () => {
            if (link.url) {
  ...
  static renderExperience(parent, experienceData) {
    ...
    experienceData.forEach((job) => {
      ...
      const title = DOMUtils.createElement("span", "job-title", {
        text: job.title,
      });
      const company = DOMUtils.createElement("span", "company", {
        text: ` — ${job.company}`,
      });
      ...
      if (job.projects) {
        job.projects.forEach((proj) => {
          ...
          const descDiv = DOMUtils.createElement("div", "project-desc", {
            text: proj.desc,
          });
      ...
      if (job.skills) {
        ...
        job.skills.forEach((skill) => {
          const span = DOMUtils.createElement("span", "skill-tag", {
            text: skill,
          });
  ...
  static renderTwoColumn(parent, data) {
    ...
    if (data.technicalSkills) {
      ...
      if (data.technicalSkills.frontend)
        createSkillLine("Frontend", data.technicalSkills.frontend);
      if (data.technicalSkills.backend)
        createSkillLine("Backend & Tools", data.technicalSkills.backend);
      if (data.technicalSkills.specializations)
        createSkillLine(
          "Specializations",
          data.technicalSkills.specializations,
        );
    ...
    if (data.education) {
      ...
      const degree = DOMUtils.createElement("div", "degree", {
      ...
      if (data.education.year) {
        const date = DOMUtils.createElement("div", "date", {
          text: data.education.year,
        });
